//  to be uploaded in the best one
...
const unsigned long SEND_INTERVAL_MS = 200;  // Check every 500ms inside slot
...
// --------------------- Sensor Mocks ------------------------
float readSulfur()  { return 5.5; }
float readPH()      { return 6.8; }
float readSodium()  { return 0.2; }
  ...
  if (cycleTime >= nodeStart && cycleTime < nodeEnd) {
    if (millis() - lastSendAttempt > SEND_INTERVAL_MS) {
      ...
      inputMessage = "{\"devEUI\":\"" + devEUI + "\"," +
                     "\"S\":" + String(readSulfur(), 1) +
                     ",\"pH\":" + String(readPH(), 2) +
                     ",\"Na\":" + String(readSodium(), 1) + "}";
